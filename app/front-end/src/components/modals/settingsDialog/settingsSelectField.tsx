...
const SettingsSelectField: React.FC<SettingsSelectFieldProps> = ({ title, description, settings, value, onChange }) => {
  return (
    <Grid container spacing={2} justifyContent='center' alignItems='center'>
      <Grid item xs={8}>
        <Typography sx={{ fontSize: '1.1rem', fontWeight: '500' }}>{title}</Typography>
        <Typography sx={{ fontSize: '0.9rem', color: 'text.secondary' }}>{description}</Typography>
      </Grid>
      <Grid item xs={4}>
